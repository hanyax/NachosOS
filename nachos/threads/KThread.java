...
public class KThread {
	...
	/**
	 * Get the called field of this thread
	 */
	...
	public static void finish() {
		...
		// Test for Join
		if (currentThread.parent != null) { // check if there is a parent
			currentThread.parent.ready(); // add parent to ready queue
		}
	...
	public void join() {
		...
		// Need to flag to check join has been called
		Lib.assertTrue(this.parent == null);

		// B is not finished, otherwise just return, joinTest1 test this
		if (this.status != statusFinished) {
			boolean intStatus = Machine.interrupt().disable();
			this.parent = currentThread; // save A for B.finsh to put in ready queue
			currentThread.sleep(); // put A to sleep
			Machine.interrupt().restore(intStatus);
		}
    ...
    private static void joinTest1() {
		...
		for (int i = 0; i < 5; i++) {
		    System.out.println ("busy..." + currentThread.getName());
    ...
    private static void joinOnRunningThread() {
		...
		A.join();
		System.out.println("After joining, A should be finished.");
		System.out.println("is it? " + (A.status == statusFinished));
		Lib.assertTrue((A.status == statusFinished), " Expected child1 to be finished.");
	}
    
    private static void joinOnItself() {
    	currentThread.join();
	}
    
    private static void joinOnJoin() {
    ...
    private static void joinMultipleTime() {
    	KThread B = new KThread( new Runnable () {
			public void run() {
			    System.out.println("B");
			}
    ...
    private static void joinEachOther() { //
    	KThread cur = KThread.currentThread();
	...
	private void run() {
		...
		called = false; // Once a thread ran, it can be reused so called should reset  
	...
	/** Thread that called current thread's join method */
	private KThread parent;
	
	/** Condition2 wake call or alarm call*/
